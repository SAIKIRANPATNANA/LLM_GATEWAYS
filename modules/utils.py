...
def get_saved_config_slug() -> str:
    return st.session_state.get("portkey_config_slug", "").strip()
...
def require_saved_config(config_key: str | None = None):
    if not get_saved_config_slug():
        st.warning(
            "This demo needs a saved Portkey config slug because inline configs are blocked in this project. "
            "Paste the `pc-...` slug from Portkey Dashboard → Configs into the sidebar, then try again."
        )
        st.stop()


def make_client(config: dict | None = None, config_key: str | None = None) -> Portkey:
    pk = get_portkey_key()
    vk = get_virtual_key()
    saved_config = get_saved_config_slug()
    if config:
        if not saved_config:
            st.warning(
                "This demo needs a saved Portkey config slug in the sidebar. "
                "Inline configs are blocked in this project."
            )
            st.stop()
        return Portkey(api_key=pk, config=saved_config)
