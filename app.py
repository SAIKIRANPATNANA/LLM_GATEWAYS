...
with st.sidebar:
    ...
    # ── Saved Config ──────────────────────────────────────────────────────
    with st.expander("Saved Config Slug", expanded=not bool(st.session_state.portkey_config_slug)):
        st.caption(
            "Paste one saved Portkey config slug here. Use the same `pc-...` slug to test whichever "
            "advanced demo you want, one at a time."
        )
        st.session_state.portkey_config_slug = st.text_input(
            "Portkey Config Slug",
            value=st.session_state.portkey_config_slug,
            placeholder="pc-...",
            key="_pc_config_input",
            help="Use the saved config ID exactly as shown in the Portkey dashboard.",
        )
        if st.session_state.portkey_config_slug:
            st.caption(f"Using saved config: `{st.session_state.portkey_config_slug}`")
